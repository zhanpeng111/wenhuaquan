...
public class PhotoAct extends Activity{
			...
			if (date != null) {
				...
				if (originalUri.getPath().toString() != null) {
					try {
						...
						op.inSampleSize = 6;
						thumbbt = BitmapFactory.decodeStream(resolver
								.openInputStream(Uri.parse(originalUri
										.toString())), null, op);
					} catch (Exception e) {
						e.printStackTrace();
	...
	class MyCamaralThread extends Thread{
		...
		public void run(){
			...
			mIMGCallBack.callback(data,thumbbt,date);
			PhotoAct.this.finish();
		}
	}
	...
	public interface IMGCallBack1{
		public void callback(String data,Bitmap bm,Intent imgdate);
	}
